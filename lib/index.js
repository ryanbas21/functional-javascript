 ...
 *  [concat description]
 *
 *  @param  {array} arr array to be concated on
 *  @param  {arguments} args spreading these args into array
 *
 *  @return {array} array of shallow concated items
 */
const concat = (arr, ...args) => {
  const value = args[0];
  if (args.length === 1) return [...arr, ...value];
  const newArgs = slice(1, args.length, args);
  return concat([...arr, ...value], newArgs);
};
/**
 *  Compose Function
...
const dropRight = (n = 0, arr) => {
  if (n === 0) return arr;
  const droppedArr = arr;
  return dropRight(n - 1, slice(0, droppedArr.length - 1, droppedArr));
};

const reject = (fn, arr) =>
  reduce((acc, item) => !fn(item) ? concat(acc, [item]) : acc, [], arr);
